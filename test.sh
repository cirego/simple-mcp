...
# Test script for simple-mcp MCP project with stdio transport
...
./server/target/debug/simple-mcp-server < test_input.json > test_output.json
...
./client/target/debug/simple-mcp-client --name "MCP Tester" > client_output.txt
...
./server/target/debug/simple-mcp-server &
SERVER_PID=$!
...
./client/target/debug/simple-mcp-client --connect --name "Connected User" > connect_output.txt
